...
from utils.ai_tools import web_search, summarize_url, generate_image
...
class AI(commands.Cog):
    ...
    async def _send_response(
        ...
        *,
        ...
        if webhook_url and self._session and not mentioned:
            try:
    ...
    async def _respond(
        ...
        *,
        ...
        # Send
        await self._send_response(message, response_text, config, mentioned=mentioned)
    ...
    async def imagine(self, interaction: discord.Interaction, prompt: str):
        ...
        embed.set_footer(text=f"Generated by Pollinations.ai for {interaction.user.display_name}")

        # Download the image and attach it directly so Discord always renders it.
        # Retry up to 3 times to give on-demand generators (Pollinations) time to finish.
        MAX_TRIES = 3
        data = None
