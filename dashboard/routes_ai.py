...
async def ai_edit(guild_id: int):
    ...
    return await render_template(
        ...
        model_groups={
            ...
            "🆓 Cerebras (Free, Fast)":[m for m, p in MODEL_PROVIDERS.items() if p == "cerebras"],
            "🆓 OpenRouter (Free models available)": [m for m, p in MODEL_PROVIDERS.items() if p == "openrouter"],
            "🆓 GitHub Models (Free)": [m for m, p in MODEL_PROVIDERS.items() if p == "github"],
            "✦ Gemini":               [m for m, p in MODEL_PROVIDERS.items() if p == "gemini"],
            "✦ DeepSeek":             [m for m, p in MODEL_PROVIDERS.items() if p == "deepseek"],
            "✦ Mistral":              [m for m, p in MODEL_PROVIDERS.items() if p == "mistral"],
